...
function Reward() {
  ...
  const fetchCommissionHistory = async () => {
    ...
    if (
      data &&
      data.hanskin?.commissionData &&
      data.sugarbear?.commissionData &&
      data.mongdies?.commissionData
    ) {
      ...
      setSnack({
        open: true,
        message: "Previous Month Data Not Found.",
        color: "error",
        icon: "warning",
      });
  ...
  const calculateCommission = () => {
    ...
    const { hanskinTotal, sugarBearTotal, mongdiesTotal } = getOrderAmountByType(
      hanskinOrder,
      sugarBearOrder,
      mongdiesOrder
    );
